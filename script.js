...
function initTypingAnimation() {
    ...
    // Get just the text content for typing
    const text = typingElement.textContent;

    // Reserve vertical space to prevent layout shift while typing
    try {
        const probe = typingElement.cloneNode(true);
        probe.style.position = 'absolute';
        probe.style.visibility = 'hidden';
        probe.style.whiteSpace = 'nowrap';
        probe.innerHTML = originalContent;
        typingElement.parentElement.appendChild(probe);
        const finalHeight = probe.offsetHeight;
        const finalWidth = probe.offsetWidth;
        typingElement.parentElement.removeChild(probe);
        typingElement.style.minHeight = finalHeight + 'px';
        // Optional: prevent wrapping during typing so height remains constant
        typingElement.style.whiteSpace = 'nowrap';
        // Reserve width so reflow doesn’t wrap midway
        typingElement.style.minWidth = finalWidth + 'px';
        typingElement.style.display = 'inline-block';
    } catch (e) {
        // Fallback: ensure some minimum height
        typingElement.style.minHeight = '1em';
        typingElement.style.display = 'inline-block';
    }

    const typingSpeedMs = 100; // lower = faster
    const pauseAfterTypedMs = 1500; // how long to show highlighted version
    ...
    const typeWriter = () => {
        if (i < text.length) {
            ...
            i++;
            setTimeout(typeWriter, typingSpeedMs);
        } else {
            // Show highlighted final text, pause, then restart
            typingElement.innerHTML = originalContent;
            typingElement.style.borderRight = 'none';
            setTimeout(startCycle, pauseAfterTypedMs);
        }
    };

    // Kick off the first cycle after a short delay
